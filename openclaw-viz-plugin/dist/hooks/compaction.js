 ...
 */
export function registerCompactionHooks(api, transport) {
    const { logger } = api;
    // 压缩前
    api.on("before_compaction", (event) => {
        const e = event;
        logger.info(`[agent-viz] before_compaction: ${e.messageCount} messages, ${e.tokenCount} tokens`);
        transport.send({
    ...
    api.on("after_compaction", (event) => {
        const e = event;
        logger.info(`[agent-viz] after_compaction: ${e.messageCount} messages, ${e.tokenCount} tokens, compacted ${e.compactedCount}`);
        transport.send({
